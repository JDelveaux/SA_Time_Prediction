...
if dataset_name != "all":
    if conv:
        class NeuralNetwork(nn.Module):
            def __init__(self):
                super().__init__()
                self.conv1 = nn.Conv1d(1, 25, 5)
                self.pool = nn.MaxPool1d(5)
                self.conv2 = nn.Conv1d(25, 107, 5)
                self.fc1 = nn.Linear(1177, 1024)
...
for t in range(epochs):
    ...
    test_loop(test_loader, model, loss_fn, output_option)
print("Done!")
